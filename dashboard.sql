...
active_campaigns AS (
    SELECT 
        ...
        COUNT(DISTINCT CONCAT(utm_source, utm_medium, utm_campaign)) AS active_campaigns
    FROM (
        SELECT 
            utm_source,
            utm_medium,
...
--    COALESCE(SUM(leads_count)::float / NULLIF(SUM(visitors_count), 0) * 100, 0) AS conversion_click_to_lead,
--    COALESCE(SUM(purchases_count)::float / NULLIF(SUM(leads_count), 0) * 100, 0) AS conversion_lead_to_purchase
--FROM 
--    conversion_data
--GROUP BY 
--    visit_date
