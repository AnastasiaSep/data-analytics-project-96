...
SELECT
    final_data.visit_date,
    final_data.utm_source,
    final_data.utm_medium,
    final_data.utm_campaign,
    SUM(final_data.visitors_count) AS visits_count,
    SUM(total_cost) AS total_cost,
    SUM(revenue) AS revenue,
    SUM(leads_count) AS leads_count,
    SUM(final_data.purchases_count) AS purchase_count,
    COALESCE(
        SUM(leads_count) / NULLIF(SUM(final_data.visitors_count), 0) * 100, 0
    ) AS conversion_r_click_lead,
    COALESCE(
        SUM(final_data.purchases_count) / NULLIF(SUM(leads_count), 0) * 100, 0
    ) AS conversion_r_lead_purchase,
    COALESCE(SUM(final_data.total_cost) / NULLIF(SUM(final_data.visitors_count), 0), 0) AS cpu,
    COALESCE(SUM(final_data.total_cost) / NULLIF(SUM(final_data.leads_count), 0), 0) AS cpl,
    COALESCE(SUM(final_data.total_cost) / NULLIF(SUM(final_data.purchases_count), 0), 0) AS cppu,
    COALESCE(
        (SUM(final_data.revenue) - SUM(final_data.total_cost))
        / NULLIF(SUM(final_data.total_cost), 0)
        * 100,
        0
    ) AS roi
FROM
